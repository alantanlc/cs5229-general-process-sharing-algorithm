...
void gps::run()
{
	...
	while (!unarrived_packets.empty() || !arrived_packets.empty())
	{
		...
		double serve_size = delta_time * speed / static_cast<double>(arrived_packets.size());
		...
		auto i = begin(arrived_packets);
		while (i != end(arrived_packets))
		{
			i->done += serve_size;
			i->to_be_done -= serve_size;

			if (abs(i->done - i->packet_size) < DBL_EPSILON)
			{
		...
		double smallest_to_be_size_size = numeric_limits<double>::max();
		for (auto &i : arrived_packets)
		{
			if (i.to_be_done < smallest_to_be_size_size)
			{
				smallest_to_be_size_size = i.to_be_done;
			}
		}

		// Derive time based on smallest to_be_done size
		current_time += (smallest_to_be_size_size * static_cast<double>(arrived_packets.size())) / this->speed;
